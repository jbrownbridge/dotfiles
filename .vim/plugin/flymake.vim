...
function! FlyMake(checker, err_regexp, warn_regexp)
  ...
  let command  = printf(a:checker, build_dir, expand('%:p:t'))
  ...
  if num_errors == 0
    call s:FlyMakeDisplay('*FlyMakeWarn*', 'Warning', messages, a:warn_regexp)
  endif
...
function! FlyMakeCloseWindows()
  " if flymake created matches go through all off them and remove
  if exists('w:havematches_flymake')
    for flymake_match in w:havematches_flymake
      call matchdelete(flymake_match)
    endfor
    unlet w:havematches_flymake 
  endif
  execute 'match none'
...
function! s:FlyMakeTemporaryDirectory(dir)
  ...
  let tmp_dir = '/tmp/flymake' . '.' . sourcefile
  call system('mkdir -p ' . tmp_dir)
"  let tmp_dir = '/tmp/flymake' "substitute(system('mktemp -d'), '\r\|\n', '', 'g')
"  call system('cp -a ' . a:dir . '/' . sourcefile . ' ' . tmp_dir)
  let command = 'cp ' . a:dir . '/' . sourcefile . ' ' . tmp_dir
  call system(command)
  return tmp_dir
endfunction
...
function! s:FlyMakeDisplay(buf, type, msg, regexp)
  ...
  if !exists('w:havematches_flymake')
    let w:havematches_flymake = []
  endif
  for key in sorted_keys
    let id = matchadd(a:type, '\%' . key . 'l')
    call add(w:havematches_flymake, id)
  endfor
